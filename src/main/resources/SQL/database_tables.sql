...
CREATE TABLE ie_eval (
    ...
    e_eval_type INTEGER NOT NULL,
    FOREIGN KEY(e_eval_type) REFERENCES ie_eval_type(et_id),
    e_pid_trainee INTEGER NOT NULL,
    FOREIGN KEY(e_pid_trainee) REFERENCES ie_person(p_id),
    e_batch INTEGER NOT NULL,
    FOREIGN KEY(e_batch) REFERENCES ie_batch(b_id)
);

CREATE TABLE ie_eval_comment (
    ec_id INTEGER PRIMARY KEY,
    ec_comment_text VARCHAR NOT NULL,
    ec_eid INTEGER NOT NULL,
    FOREIGN KEY(ec_eid) REFERENCES ie_eval(e_id)
);

CREATE TABLE ie_subject( s_id INTEGER PRIMARY KEY,
    s_subject VARCHAR(150) NOT NULL
);

CREATE TABLE ie_question_pool( qp_id INTEGER PRIMARY KEY,
...
CREATE TABLE ie_question_eval (
    ...
    qe_eid INTEGER NOT NULL,
    FOREIGN KEY(qe_eid) REFERENCES ie_eval(e_id),
    qe_qpid INTEGER NOT NULL,
    FOREIGN KEY(qe_qpid) REFERENCES ie_question_pool(qp_id)
);

CREATE TABLE ie_question_comment (
    qc_id INTEGER PRIMARY KEY,
    qc_comment_text VARCHAR NOT NULL,
    qc_eid INTEGER NOT NULL,
    FOREIGN KEY(qc_eid) REFERENCES ie_question_eval(qe_id)
);
